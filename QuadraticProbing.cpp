...
int QuadraticProbing::getBalance(std::string id) {
    if(doesExist(id)){
        ...
        for(int j=0;j<bankStorage1d.size();j++){
            if(bankStorage1d[(j*j+ind)%100000].id==id && bankStorage1d[(j*j+ind)%100000].balance!=-1){
                return bankStorage1d[(j*j+ind)%100000].balance;
            }
        }
    }
    return -1;
...
void QuadraticProbing::addTransaction(std::string id, int count) {
    if(doesExist(id)){
        ...
        for(int j=0;j<bankStorage1d.size();j++){
            if(bankStorage1d[(j*j+ind)%100000].id==id && bankStorage1d[(j*j+ind)%100000].balance!=-1){
                bankStorage1d[(j*j+ind)%100000].balance=bankStorage1d[(j*j+ind)%100000].balance+count;
            }
        }
    }
    else{
...
bool QuadraticProbing::deleteAccount(std::string id) {
    int ind=hash(id);
    for(int j=0;j<bankStorage1d.size();j++){
        if(bankStorage1d[(j*j+ind)%100000].id==id){
            bankStorage1d[(j*j+ind)%100000].balance=-1;
            size--;
            return true;
        }
    }
